    ...
    : QWidget(parent)
{
    ...
    m_enemies.append(new Enemy(QPoint(300, GAME_HEIGHT - 150))); // 固定位置敌人
...
void MainScene::paintEvent(QPaintEvent *){
    ...
    for(const auto bullet : m_bullets) {
      ...
      painter.drawPixmap(m_doro.m_X, m_doro.m_Y, m_doro.m_facingRight ? m_doro.m_doroRight : m_doro.m_doroLeft);
      ...
      // 绘制分数
      painter.setPen(Qt::white);
      painter.drawText(20, 70, QString("Score: %1").arg(m_score));
...
void MainScene::keyPressEvent(QKeyEvent* event) {
    ...
    if (event->key() == Qt::Key_Space && m_doro.standingEnemy) {
        // 从敌人身上起跳
        m_doro.standingEnemy = nullptr;
        m_doro.m_velocityY = -20.0f;
    }
...
void MainScene::spawnEnemy() {
    if(QRandomGenerator::global()->bounded(100) < 20) {
        int x = QRandomGenerator::global()->bounded(GAME_WIDTH - 100);
        int y = GAME_HEIGHT - 150;
        m_enemies.append(new Enemy(QPoint(x, y)));
    }
    QTimer::singleShot(QRandomGenerator::global()->bounded(500,1500), this, &MainScene::spawnEnemy);
}
    ...
    foreach (Enemy* enemy, m_enemies) {
        ...
        if (doroFeet.intersects(enemyHead)) {
            ...
            if (overlap >= 1 && overlap <= 25 && m_doro.m_velocityY >= 0) {
                m_doro.standingEnemy = enemy;
                m_doro.standingOffsetX = m_doro.m_X - enemy->position.x();
                m_doro.m_Y = enemy->position.y() - m_doro.m_Rect.height() + 3;
                m_doro.m_velocityY = 0;
                m_doro.m_isGrounded = true;
